...
fuel_type = st.selectbox("Вид топлива", ['Дизель', 'Бензин', 'Бензин + газ', 'Газ', 'Электричество'])
...
input_data = np.array([[model_name, year, transmission_encoded, fuel_type_encoded, city_encoded]])

# Важно: Убедитесь, что входные данные масштабированы так же, как при обучении
# Пример: input_data_scaled = scaler.transform(input_data) если использовалась нормализация
